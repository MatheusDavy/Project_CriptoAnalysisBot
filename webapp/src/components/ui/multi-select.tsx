...
const multiSelectVariants = cva("m-1 transition-all duration-300 ease-in-out", {
	variants: {
		variant: {
			default: "border-acent text-accent bg-card hover:bg-accent-500 bg-accent-500 border-1 border-accent",
			secondary:
				...
				<Popover
					...
					<PopoverTrigger asChild>
						<Button
							...
							className={cn(
								"flex p-1 rounded-md border min-h-10 h-auto items-center justify-between bg-bg-100 hover:bg-inherit [&_svg]:pointer-events-auto focus:bg-accent-500",
							...
							{selectedValues.length > 0 ? (
								...
								<div className="flex items-center justify-between w-full mx-auto text-accent-300 normal-case">
									<span className="text-sm text-inherit mx-3">
										{placeholder}
									</span>
									<ChevronDown className="h-4 cursor-pointer text-inherit mx-2" />
								</div>
							)}
						</Button>
					</PopoverTrigger>
					...
					>
						<Command>
							...
							<CommandList
								className={cn(
									"max-h-[40vh] overflow-y-auto multiselect-scrollbar bg-bg-200",
